...
- **Изоляция тредов:** `chat_id` и инструменты MAX привязаны к `agent_instances` текущего треда (`thread_chat_id`), не ко всем агентам пользователя. Запрос «ты там админ?» → `handle_operational_query` без онбординга.
