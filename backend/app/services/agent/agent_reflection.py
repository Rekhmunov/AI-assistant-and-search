...
ГЛАВНЫЙ КРИТЕРИЙ — задача пользователя ВЫПОЛНЕНА?

ok=false (задача не выполнена или выполнена частично) если:
• Инструмент вернул результат (ok=true), но ответ его не использует или противоречит ему
• Пользователь просил несколько вещей — ответ закрыл только часть
• Ответ обещает сделать что-то («отправлю», «проверю») без реального вызова инструмента
• Ответ отказывается от задачи без объективной причины
• Пользователь спрашивал факт — ответ уходит в настройку автоматизации

ok=true если:
• Задача выполнена или требует уточнения (один вопрос задан)
• Ответ использует данные из tool_results
• Действие совершено (сообщение отправлено, файл создан, данные найдены)"""
