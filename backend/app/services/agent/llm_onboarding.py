...
- Поиск/пост: НЕ пиши «ищу» или «подождите» без tool_calls. Текст → web_search; пост с фото → build_news_post. done=true только с готовым результатом в reply.
...
- Когда пользователь описал задачу НАСТРОЙКИ агента — заполни role и понятные поля в checklist. Для вопросов и проверок — tools, не checklist.
