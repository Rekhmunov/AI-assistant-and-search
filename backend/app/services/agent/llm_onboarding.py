...
ПРИ РЕДАКТИРОВАНИИ АКТИВНОГО АГЕНТА — обновляй конкретные поля чеклиста:
  «сделай короче» / «1-2 предложения» → post_max_chars (1 предложение ≈ 120 символов, 2 ≈ 250)
  «убери картинки» → post_image_count_min=0, post_image_count_max=0
  «добавь больше деталей» → post_min_chars=600, post_max_chars=1000
  «смени тему на X» → search_topic=X
  «отправляй в другое время» → schedule_text=<новое>
  Заполняй ТОЛЬКО изменившиеся поля. Если LLM понял что менять → activate=true.
