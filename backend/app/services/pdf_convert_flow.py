...
async def stream_pdf_convert_turn(
    ...
    if not original_bytes:
        yield sse_event("error", {"code": "convert_failed", "message": "Не удалось прочитать PDF-файл."})
        await clear_search_pending(redis_client, thread.id)
        return
    ...
    try:
        ...
        for page_num in range(total_pages):
            ...
            if total_size + len(jpg_bytes) > max_zip_bytes:
                # Лимит ZIP достигнут
                converted = len(pages_data)
                if converted == 0:
                    # Даже первая страница не влезает
                    plan_label = "Pro" if user.plan == Plan.PRO else "Free"
                    yield sse_event("error", {
                        "code": "zip_size_limit",
                        "message": (
                            f"Файл слишком большой для конвертации на тарифе {plan_label} "
                            f"(лимит {max_zip_mb} МБ). "
                        ),
                    })
                    await clear_search_pending(redis_client, thread.id)
                    return
                # Конвертируем то что влезло
                break
        ...
        logger.warning("pdf convert failed: %s", exc)
        yield sse_event("error", {"code": "convert_failed", "message": "Не удалось конвертировать PDF. Попробуйте позже."})
        await clear_search_pending(redis_client, thread.id)
        return
    ...
    # Обновить download endpoint — добавить "converted" в allowlist сделано ранее
    ...
    if converted_pages == 1:
        result_text = (
            f"\n\n✅ Готово!\n"
            f"1 страница → JPG ({format_size(file_size)})\n"
            f"Файл хранится 24 часа."
        )
    elif truncated:
        result_text = (
            f"\n\n✅ Конвертировано {converted_pages} из {total_pages} страниц — "
            f"достигнут лимит {max_zip_mb} МБ ({format_size(total_size)}).\n"
            f"ZIP-архив: {format_size(file_size)}\n"
            f"Файл хранится 24 часа."
        )
    else:
        result_text = (
            f"\n\n✅ Готово!\n"
            f"{converted_pages} страниц → ZIP-архив ({format_size(file_size)})\n"
            f"Файл хранится 24 часа."
        )
    ...
    if not thread_id:
        thread.title = f"PDF → JPG · {pdf_file.filename or 'файл'}"
    await db.commit()
