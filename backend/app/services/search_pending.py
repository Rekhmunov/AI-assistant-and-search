"""Redis-состояние активного поиска по треду (для polling после ухода со страницы)."""

from __future__ import annotations

import json
import logging
import uuid
from datetime import datetime, timezone
from typing import Any

import redis.asyncio as redis

logger = logging.getLogger(__name__)

PENDING_KEY_PREFIX = "search:pending:"
PENDING_TTL_SEC = 15 * 60
STALE_AFTER_SEC = 45
# Макс. время «живого» pending без завершения — иначе зависший (reload / обрыв SSE).
PENDING_ZOMBIE_SEC = 20

SearchPhase = str  # routing | searching | answering | image_generating | document_generating


def _key(thread_id: uuid.UUID | str) -> str:
    return f"{PENDING_KEY_PREFIX}{thread_id}"


async def set_search_pending(
    redis_client: redis.Redis,
    thread_id: uuid.UUID,
    *,
    user_message_id: uuid.UUID,
    phase: SearchPhase,
    needs_search: bool | None = None,
    intent: str | None = None,
    custom_status: str | None = None,
) -> None:
    payload: dict[str, Any] = {
        "user_message_id": str(user_message_id),
        "phase": phase,
        "started_at": datetime.now(timezone.utc).isoformat(),
    }
    if needs_search is not None:
        payload["needs_search"] = needs_search
    if intent:
        payload["intent"] = intent
    if custom_status:
        payload["custom_status"] = custom_status
    try:
        await redis_client.set(_key(thread_id), json.dumps(payload), ex=PENDING_TTL_SEC)
    except Exception:
        logger.warning("set_search_pending failed for thread %s", thread_id, exc_info=True)


async def update_search_pending(
    redis_client: redis.Redis,
    thread_id: uuid.UUID,
    *,
    phase: SearchPhase | None = None,
    needs_search: bool | None = None,
    custom_status: str | None = None,
) -> None:
    try:
        raw = await redis_client.get(_key(thread_id))
        if not raw:
            return
        data = json.loads(raw)
        if phase is not None:
            data["phase"] = phase
        if needs_search is not None:
            data["needs_search"] = needs_search
        if custom_status is not None:
            data["custom_status"] = custom_status
        data["updated_at"] = datetime.now(timezone.utc).isoformat()
        await redis_client.set(_key(thread_id), json.dumps(data), ex=PENDING_TTL_SEC)
    except Exception:
        logger.warning("update_search_pending failed for thread %s", thread_id, exc_info=True)


async def clear_search_pending(redis_client: redis.Redis, thread_id: uuid.UUID) -> None:
    try:
        await redis_client.delete(_key(thread_id))
    except Exception:
        logger.warning("clear_search_pending failed for thread %s", thread_id, exc_info=True)


def pending_active_seconds(pending_raw: dict[str, Any]) -> float | None:
    started = pending_raw.get("updated_at") or pending_raw.get("started_at")
    if not started:
        return None
    try:
        raw = str(started).replace("Z", "+00:00")
        dt = datetime.fromisoformat(raw)
        if dt.tzinfo is None:
            dt = dt.replace(tzinfo=timezone.utc)
        return (datetime.now(timezone.utc) - dt).total_seconds()
    except (TypeError, ValueError):
        return None


def is_pending_zombie(pending_raw: dict[str, Any] | None) -> bool:
    if not pending_raw:
        return False
    age = pending_active_seconds(pending_raw)
    if age is None:
        return False
    return age >= PENDING_ZOMBIE_SEC


async def get_search_pending(redis_client: redis.Redis, thread_id: uuid.UUID) -> dict[str, Any] | None:
    try:
        raw = await redis_client.get(_key(thread_id))
        if not raw:
            return None
        data = json.loads(raw)
        return data if isinstance(data, dict) else None
    except Exception:
        logger.warning("get_search_pending failed for thread %s", thread_id, exc_info=True)
        return None
