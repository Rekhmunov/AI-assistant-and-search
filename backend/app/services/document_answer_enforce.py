...
def document_request_prompt_addon(query: str, *, is_legal_doc: bool = False) -> str:
    ...
    lines = [
        ...
        "2. Каждый раздел должен содержать полный юридически корректный текст, минимум 3–5 абзацев.",
        "3. Не используй заглушки вида «перечислить условия» — пиши конкретный текст.",
    ]
    if is_legal_doc:
        lines.append(
            "4. Используй актуальные нормы из источников [n] с номерами статей (ст. 437 ГК РФ и т.д.)."
        )
        lines.append(
            "5. ОБЯЗАТЕЛЬНЫЕ РАЗДЕЛЫ для договоров: Предмет; Цена и порядок расчётов; "
            "Права и обязанности сторон; Ответственность; Срок действия; Реквизиты."
        )
    return "\n".join(lines)
