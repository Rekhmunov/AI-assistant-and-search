...
- image_edit — пользователь просит ИЗМЕНИТЬ изображение: либо сгенерированное ранее в треде, либо прикреплено ОДНО фото с инструкцией изменить его. Маркеры: «сделай ч/б», «убери фон», «измени цвет», «добавь элемент», «перекрась», «измени стиль», «сделай другим». НЕ использовать если пользователь просит ОПИСАТЬ или ПРОАНАЛИЗИРОВАТЬ фото (что на фото → search_rag). НЕ использовать если прикреплены 2+ фото (→ image_compose).
