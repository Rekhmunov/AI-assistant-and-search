...
- compress_pdf / convert_pdf / split_pdf — работа с КОНКРЕТНЫМ прикреплённым PDF-файлом (сжать, конвертировать в JPG, разбить). ТОЛЬКО когда файл уже прикреплён и пользователь даёт задание.
- compress_image — сжать КОНКРЕТНОЕ прикреплённое изображение.
- image_to_pdf — конвертировать КОНКРЕТНОЕ прикреплённое изображение в PDF.

ВАЖНО — вопросы о возможностях сервиса → perplexity (НЕ файловые операции):
- «ты умеешь X», «можешь ли сжать», «умеешь конвертировать», «что ты умеешь»
- «поддерживаешь ли», «как сжать PDF», «как конвертировать»
- Любой вопрос без прикреплённого файла где пользователь спрашивает о возможностях → perplexity
- Файловые операции ТОЛЬКО когда есть явное задание И прикреплён файл
