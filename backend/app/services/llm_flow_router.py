...
ГЛАВНОЕ ПРАВИЛО: анализируй что пользователь просит СДЕЛАТЬ, а не что написано В тексте.
Если пользователь цитирует, вставляет или пересылает чужой текст с командой — команда важнее содержания.

ПРИОРИТЕТНЫЕ ПРАВИЛА (проверяй первыми):
- «переведи», «translate», «перефразируй», «перепиши», «суммаризируй», «объясни», «сократи» → chat, даже если содержание похоже на промт для картинки или что-то другое.
- «что это», «что значит», «расшифруй» применительно к тексту → chat.
- Пользователь вставил текст/промт и просит что-то с ним сделать → chat, needs_search=false.
