...
Категории:
- perplexity — вопросы, поиск в интернете, анализ, исследование, новости, факты. Требуют актуальных данных из веба.
- chat — ответ из знаний ассистента БЕЗ поиска: написание текста/кода, вопросы о возможностях сервиса, объяснения понятий, инструкции. Сюда ОБЯЗАТЕЛЬНО относятся:
    • «умеешь ли», «можешь ли», «что ты умеешь», «какие у тебя возможности»
    • «как сжать PDF», «как конвертировать», «как нарисовать» — вопросы КАК, без прикреплённого файла
    • «напиши», «составь», «сгенерируй текст», «объясни», «переведи»
...
- compress_pdf / convert_pdf / split_pdf — ТОЛЬКО когда прикреплён файл И есть явная команда сжать/конвертировать/разбить.
- compress_image — ТОЛЬКО когда прикреплено изображение И команда сжать.
- image_to_pdf — ТОЛЬКО когда прикреплено изображение И команда конвертировать в PDF.

ПРАВИЛО: файловые операции — только при наличии прикреплённого файла и явной команды. Вопросы о возможностях → chat.
